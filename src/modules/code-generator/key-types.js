// Original file is at https://www.aarongreenlee.com/blog/list-of-non-printable-keys-for-keyboard-events-when-using-event-key/
...
const nonPrintableKeyMap = Object.values(nonPrintableKeys).reduce((map, keys) => {
...
export const isPrintableKey = key => key && !nonPrintableKeyMap[key]

export const isIMECompositionKey = key => key && nonPrintableKeys.imeCompositionKeys.includes(key)

export const isModifierKey = key =>
  nonPrintableKeys.modifier.includes(key) || nonPrintableKeys.legacyModifier.includes(key)
